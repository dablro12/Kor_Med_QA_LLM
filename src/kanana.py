...
class Kanana:
    """
    Kanana wrapper (AutoModelForCausalLM)
    - 캐시/오프라인 지원
    - TEXT-only 지원
    - count_tokens() 제공
    """
    ...
    def __init__(self, hg_model_id: str, device="cuda", cache_dir=None):
        ...
        print(f"[Kanana] cache_dir = {cache_dir}")
        print(f"[Kanana] model cache dir = {model_cache_dir}")
        print(f"[Kanana] local_files_only = {self.local_files_only}")
    ...
    def run(self, prompt_or_messages, max_new_tokens=2048, temperature=0, top_p=1.0, num_beams=1, do_sample=False, **kwargs):
        ...
        if hasattr(self.tokenizer, "apply_chat_template"):
            ...
            input_text = "You are a clinical doctor.\n" + (prompt_or_messages if isinstance(prompt_or_messages, str) else messages[-1]["content"])
...
if __name__ == "__main__":
    model = Kanana(
        hg_model_id="kakaocorp/kanana-1.5-2.1b-instruct-2505",
        device="cuda",
