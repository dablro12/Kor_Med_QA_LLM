...
class Gemma:
    """
    Gemma wrapper (text-generation pipeline)
    - 캐시/오프라인 지원
    - TEXT-only 지원
    - count_tokens() 제공
    """
    ...
    def __init__(self, hg_model_id: str, device="cuda", cache_dir=None):
        ...
        print(f"[Gemma] cache_dir = {cache_dir}")
        print(f"[Gemma] model cache dir = {model_cache_dir}")
        print(f"[Gemma] local_files_only = {self.local_files_only}")
...
if __name__ == "__main__":
    model = Gemma(
        hg_model_id="google/gemma-3-4b-it",
        device="cuda",
